<template>
  ...
  <div>
    <main id="" class="main" style="margin-top: 32px; margin-left: 103px; margin-right: 100px">
      <div class="reading">
        <div class="menuChapter bg-danger-subtle p-1 ps-2 d-flex justify-content-center sticky-top"
          style="top: 0px; margin-top: -23px; z-index: 99">
          <RouterLink :to="{ path: '/' }">
          ...
          <RouterLink :to="{ path: detailPath }"><button class="btn btn-success m-1 backDetailStory">
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
          ...
          <select class="form-select w-25" aria-label="Default select example" @change="changeChapter"
            :value="chapterId">
            <option v-for="chapter in chapters" :key="chapter?.id" :value="chapter?.id">Chapter {{ chapter.so }}
            </option>
          </select>
